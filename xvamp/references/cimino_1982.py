...
# initialize data and interpolators
...
for tbl, part in zip([t_real, t_imaginary], ["Real", "Imaginary"]):
    for band in ["s", "x"]:
        ...
        new_vals = np.clip(
            np.array(
                [
                    np.polynomial.Polynomial.fit(subtemps, interp_vals[ic, :], deg=2)(
                        EXTRAPOLATE_DOWN_TO
                    )
...
def get_h2s04_rel_permittivity(
    ...
    if wavelength == "s":
        ...
        eps_prime_r = get_real_sband(ct)
        eps_dprime_r = get_imaginary_sband(ct)
        return (
        ...
        eps_prime_r = get_real_xband(ct)
        eps_dprime_r = get_imaginary_xband(ct)
        return (
            Quantity(eps_prime_r, u.dimensionless_unscaled),
            Quantity(eps_dprime_r, u.dimensionless_unscaled),
        )
        ...
        real_s = get_real_sband(ct)
        real_x = get_real_xband(ct)
        imaginary_s = get_imaginary_sband(ct)
        imaginary_x = get_imaginary_xband(ct)
